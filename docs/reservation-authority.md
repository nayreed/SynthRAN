...
`fresh` requires proven calendar authority first and uses two phases. Phase 1 proves allocation authority for **every selected SOP node before any image/reset mutation occurs**. If one selected node cannot be allocated, no selected node has been reimaged. Fresh allocations are tagged with a SynthRAN-generated POS `result_folder` identity retained only with the exact POS calendar authority. On a rerun of that same calendar event, an already-active allocation is reused without release/reacquire only when live provider metadata (`pos allocations show`) proves the same retained allocation ID and result-folder identity. A bare `already allocated` response, missing metadata, a different allocation ID/result folder, a different calendar event, or a direct caller without persisted authority all keep the fail-closed reclaim/reacquire path. This removes verified self-churn without treating calendar ownership as allocation ownership.
...
Deployment latency evidence is retained separately in `results/<run>/phase-timings.json`. The controller records UTC start/completion timestamps plus monotonic elapsed seconds for SOP and R2Lab reservation work, per-node fresh image staging, boot-parameter mutation, POS reset and SSH readiness, Kubernetes bootstrap, R2Lab RRU setup, physical UE setup, selected 5G stack deployment, and final accepted-testbed verification. Bootstrap boot-profile correction uses the same per-node `boot_parameter_mutation` phase and also records the ordinary `bootstrap_reboot` duration separately, so an in-place boot-drift repair is not hidden inside later provisioning time. Concurrent node workers append under a file lock. If the controller exits while a phase is still open, the exit handler closes it as `failed` or `incomplete` rather than leaving an ambiguous running record.
...
Task #124 also has a consolidated regression matrix covering preserve failure-before-R2Lab mutation, healthy preserve zero mutation, bootstrap in-place repairability, bootstrap explicit requires-fresh blocking, the fresh known-clean POS sequence, and multi-node fresh concurrency/evidence. Phase-timing behavior and instrumentation have their own contract check.
